...
export const checkAndRunRebalancing = async () => {
  try {
    ...
    const daysSinceLastRun =
      (now.getTime() - new Date(lastRun.completedAt).getTime()) /
      (1000 * 60 * 60 * 24);

    if (daysSinceLastRun >= REBALANCING_INTERVAL_DAYS) {
      console.log(
        `[Rebalancing Job] Triggering bi-weekly rebalancing (${daysSinceLastRun.toFixed(1)} days since last run)...`,
      );
      await runRebalancing();
    } else {
      console.log(
        `[Rebalancing Job] Skipping run. Last completed ${daysSinceLastRun.toFixed(1)} days ago.`,
      );
    }
  } catch (error) {
