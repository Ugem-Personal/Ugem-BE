...
export const createIncident = async (
  ...
  const incident = await prisma.merchantIncident.create({
    data: {
      ...
      severity: enumValue(
        input.severity ?? IncidentSeverity.Medium,
  ...
  await audit(
    actor,
    "MERCHANT_INCIDENT_CREATED",
    "MerchantIncident",
    incident.id,
    { merchantId, severity: incident.severity, type: incident.type },
  );
  return incident;
};
...
export const reviewIncident = async (
  ...
  const updated = await prisma.$transaction(async (transaction) => {
    const next = await transaction.merchantIncident.update({
      ...
      data: {
        ...
        severity,
        adminDecision:
          typeof input.adminDecision === "string"
            ? input.adminDecision.trim()
            : null,
        resolution:
          typeof input.resolution === "string" ? input.resolution.trim() : null,
  ...
  await audit(
    actor,
    "MERCHANT_INCIDENT_REVIEWED",
    "MerchantIncident",
    incidentId,
    { status, severity, merchantId: incident.merchantId },
  );
  return updated;
};
...
export const reviewClaim = async (
  ...
  const status = enumValue(
    input.status,
    Object.values(MerchantClaimStatus),
    "status",
  );
  const updated = await prisma.$transaction(async (transaction) => {
    ...
    const result = await transaction.merchantClaim.update({
      ...
      data: {
        status,
        decision:
          typeof input.decision === "string" ? input.decision.trim() : null,
        reviewedByUserId: actor.userId,
        reviewedAt: new Date(),
      },
    });
...
export const reviewRemoval = async (
  actor: AuditActor,
  id: string,
  input: Record<string, unknown>,
) => {
  ...
  const status = enumValue(
    input.status,
    Object.values(MerchantRemovalStatus),
    "status",
  );
  const updated = await prisma.$transaction(async (transaction) => {
  ...
  await audit(
    actor,
    "MERCHANT_REMOVAL_REVIEWED",
    "MerchantRemovalRequest",
    id,
    { status, merchantId: request.merchantId },
  );
  return updated;
};
...
export const createSuggestion = async (
  ...
  const suggestion = await prisma.restaurantSuggestion.create({
    data: {
      ...
      recommendedDish:
        typeof input.recommendedDish === "string"
...
export const reviewSuggestion = async (
  ...
  const status = enumValue(
    input.status,
    Object.values(RestaurantSuggestionStatus),
    "status",
  );
  ...
  await audit(
    actor,
    "RESTAURANT_SUGGESTION_REVIEWED",
    "RestaurantSuggestion",
    id,
    { status },
  );
...
export const getFunnelSummary = async () => {
  ...
  return Object.fromEntries(
    rows.map((row) => [row.eventType, row._count._all]),
  );
};

export const listFeePolicies = async () =>
  prisma.monetizationFeePolicy.findMany({
    orderBy: [{ feeType: "asc" }, { effectiveAt: "desc" }],
  });

export const upsertFeePolicy = async (
  actor: AuditActor,
  input: Record<string, unknown>,
) => {
  const feeType = enumValue(
    input.feeType,
    Object.values(MonetizationFeeType),
    "feeType",
  );
  const amount =
  ...
  await audit(
    actor,
    "MONETIZATION_FEE_POLICY_CREATED",
    "MonetizationFeePolicy",
    policy.id,
    { feeType, configured: amount !== null },
  );
  return policy;
};
