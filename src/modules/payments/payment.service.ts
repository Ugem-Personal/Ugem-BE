...
export const submitBill = async (
  ...
  if (
    ...
    throw new AppError(409, "Chỉ có thể tạo bill sau khi món đã được lên bàn");
  }
...
const extractOrderId = async (content?: string): Promise<string | null> => {
  ...
  const hex8Match = content.match(
    /(?:THANH\s*TOAN\s*DON|UGEM)[\s-]*([0-9a-f]{8})/i,
  );
  if (hex8Match?.[1]) {
...
export const processSepayWebhook = async (input: SepayWebhookInput) => {
  ...
  const reference =
    input.referenceCode?.trim() || (input.id ? `SEPAY-${input.id}` : "");
  ...
  if (
    !order.merchant.bankTransferEnabled ||
    !receivingAccount ||
    !merchantAccount ||
