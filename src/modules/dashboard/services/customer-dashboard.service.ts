...
export const getCustomerSpendingByYear = async (
  customerId: string,
  year: number,
) => {
  const customer = await prisma.customer.findUnique({
  ...
  const totalSpending = months.reduce(
...
export const getCustomerOrderGrowthByYear = async (
  customerId: string,
  year: number,
) => {
  const customer = await prisma.customer.findUnique({
...
export const getCustomerPaymentStatisticsByYear = async (
  customerId: string,
  year: number,
) => {
  const customer = await prisma.customer.findUnique({
  ...
  const totalSpending = Object.values(paymentMethods).reduce(
    (total, method) => total + method.spending,
    0,
  );
...
export const getCustomerFavoriteMerchantsByYear = async (
  customerId: string,
  year: number,
  limit: number,
) => {
  ...
  const totalSpending = merchants.reduce(
    (total, merchant) => total + merchant.totalSpending,
    0,
  );

  const completedOrders = merchants.reduce(
    (total, merchant) => total + merchant.completedOrders,
    0,
  );
    ...
    .map((merchant, index) => ({
      ...
      averageOrderValue:
        merchant.completedOrders > 0
          ? Number(
              (merchant.totalSpending / merchant.completedOrders).toFixed(2),
            )
          : 0,
  ...
  const favoriteMerchant =
    favoriteMerchants.length > 0 ? favoriteMerchants[0]! : null;
...
export const getCustomerWeekdayStatisticsByYear = async (
  customerId: string,
  year: number,
) => {
  const customer = await prisma.customer.findUnique({
  ...
  const totalSpending = weekdays.reduce(
    (total, weekday) => total + weekday.spending,
    0,
  );
  ...
  const favoriteWeekday =
    totalOrders > 0
      ? weekdays.reduce(
          (currentFavorite, weekday) =>
            weekday.totalOrders > currentFavorite.totalOrders
              ? weekday
              : currentFavorite,
          weekdays[0]!,
        )
...
export const getCustomerPeakHoursByYear = async (
  customerId: string,
  year: number,
) => {
  const customer = await prisma.customer.findUnique({
  ...
  const favoriteHour =
    totalOrders > 0
      ? hours.reduce(
          (currentFavorite, hour) =>
            hour.totalOrders > currentFavorite!.totalOrders
              ? hour
              : currentFavorite,
          hours[0],
        )
...
export const getCustomerOrderPerformanceByYear = async (
  customerId: string,
  year: number,
) => {
  const customer = await prisma.customer.findUnique({
  ...
  const highestSpendingMonth =
    summary.totalOrders > 0
      ? months.reduce(
          (currentHighest, month) =>
            month!.spending > currentHighest!.spending ? month : currentHighest,
          months[0],
        )
      : null;
  ...
  const mostActiveMonth =
    summary.totalOrders > 0
      ? months.reduce(
          (currentHighest, month) =>
            month.totalOrders > currentHighest!.totalOrders
              ? month
              : currentHighest,
          months[0],
        )
...
export const getCustomerDailySpending = async (
  customerId: string,
  ...
  const highestSpendingDay =
    summary.totalSpending > 0
      ? days.reduce(
          (currentHighest, day) =>
            day.spending > currentHighest!.spending ? day : currentHighest,
          days[0],
        )
      : null;
