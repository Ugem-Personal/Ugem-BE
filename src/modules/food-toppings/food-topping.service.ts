...
const checkFoodOwnership = async (merchantId: string, foodId: string) => {
  ...
  if (food.merchantId !== merchantId) {
    throw new AppError(
      403,
      "Bạn không có quyền quản lý topping của món ăn này",
    );
  }
...
export const getToppingsByFood = async (
  foodId: string,
...
export const deleteTopping = async (merchantId: string, toppingId: string) => {
  const topping = await prisma.foodTopping.findUnique({
    where: {
      id: toppingId,
    },
