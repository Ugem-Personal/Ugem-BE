...
export const createOrder = async (
  ...
  if (input.voucherCode?.trim()) {
    ...
    if (!voucherTx) {
      throw new AppError(
        400,
        "Mã voucher tích điểm không hợp lệ hoặc không thuộc về bạn",
      );
    }
    ...
    if (alreadyUsed) {
      throw new AppError(
        400,
        "Voucher này đã được sử dụng cho một đơn hàng khác",
      );
    }
    ...
    if (subtotal < minOrder) {
      throw new AppError(
        400,
        `Đơn hàng cần đạt tối thiểu ${minOrder.toLocaleString("vi-VN")}đ để dùng voucher này`,
      );
    }
  }
