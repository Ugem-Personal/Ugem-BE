...
export const createReviewerCommission = async (orderId: string) => {
  ...
  const earningTransaction = await prisma.$transaction(async (transaction) => {
    const currentAggregate =
      await transaction.reviewerEarningTransaction.aggregate({
        where: {
...
export const createReviewerBookingCommission = async (bookingId: string) => {
  ...
  const earningTransaction = await prisma.$transaction(async (transaction) => {
    ...
    const successfulOrderAggregate =
      await transaction.affiliateLink.aggregate({
        where: { reviewerId: booking.affiliateLink!.reviewerId },
        _sum: { successfulOrders: true },
      });
  ...
  await createNotification({
    ...
    message: `Bạn đã nhận được ${commission.toLocaleString(
      "vi-VN",
    )} VNĐ hoa hồng (${commissionPercent}% hạng ${currentRank || "Bronze"}) từ một lượt đặt bàn thành công.`,
    referenceId: earningTransaction.id,
    referenceType: "ReviewerEarningTransaction",
  });
