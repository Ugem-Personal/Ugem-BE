...
const mapAffiliateLink = (link: any) => ({
  id: link.id,
...
const createLinkCode = (): string => {
  return crypto.randomBytes(8).toString("hex").toUpperCase();
};

const hashIp = (ipAddress?: string): string | null => {
  if (!ipAddress) {
    return null;
  }
...
const buildAffiliateShareUrl = (linkCode: string): string => {
...
export const getMyAffiliateLinks = async (reviewerId: string) => {
...
export const updateAffiliateLinkStatus = async (
  reviewerId: string,
  linkId: string,
  input: UpdateAffiliateLinkStatusInput,
) => {
...
export const trackAffiliateLink = async (
  code: string,
  context: {
...
export const getReviewerEarnings = async (
  reviewerId: string,
  query: AffiliateEarningsQuery,
) => {
  ...
  const commissionOrderCount = links.reduce(
    (total, link) => total + link.successfulOrders,
    0,
  );

  const [allTransactionSummary, transactions, totalItems, pointTransactions] =
  ...
  const currentEarnings =
    allTransactionSummary.length > 0
      ? Number(
          allTransactionSummary[allTransactionSummary.length - 1]!
            .earningsAfter,
        )
      : 0;
