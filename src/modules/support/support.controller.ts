...
function getUserId(req: Request) {
  if (!req.user?.UserId) throw new AppError(401, "Không xác định được người dùng");
  return req.user.UserId;
}

function getMerchantId(req: Request) {
  if (!req.user?.MerchantId) throw new AppError(403, "Tài khoản không có MerchantId");
  return req.user.MerchantId;
}

function getId(req: Request) {
  const id = req.params.id;
  if (typeof id !== "string" || !id) throw new AppError(400, "Thiếu mã yêu cầu hỗ trợ");
  return id;
}
...
export const getMerchantTickets = asyncHandler(async (req: Request, res: Response) => {
...
export const getMerchantTicket = asyncHandler(async (req: Request, res: Response) => {
...
export const addMerchantMessage = asyncHandler(async (req: Request, res: Response) => {
  const ticket = await supportService.addMerchantMessage(
    getMerchantId(req),
    getUserId(req),
    getId(req),
    req.body,
...
export const updateMerchantStatus = asyncHandler(async (req: Request, res: Response) => {
  const ticket = await supportService.updateMerchantStatus(
    getMerchantId(req),
    getId(req),
    req.body,
  );
...
export const getStaffTickets = asyncHandler(async (req: Request, res: Response) => {
  const tickets = await supportService.getStaffTickets({ status: req.query.status as any });
  return sendSuccess(res, { message: "Lấy danh sách hỗ trợ thành công", data: tickets });
});

export const getStaffTicket = asyncHandler(async (req: Request, res: Response) => {
...
export const addStaffMessage = asyncHandler(async (req: Request, res: Response) => {
