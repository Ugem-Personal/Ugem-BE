...
const getTestDatabaseUrl = () => {
  ...
  if (!/^[A-Za-z0-9_]+$/.test(testDatabase) || !testDatabase.endsWith("_test")) {
    throw new Error(
      `E2E từ chối database '${testDatabase}'. Tên database phải kết thúc bằng _test.`,
    );
  }
...
const ensureTestDatabase = async (
  ...
  try {
    const databases = await adminPrisma.$queryRawUnsafe<Array<{ exists: boolean }>>(
...
const run = async () => {
  ...
  const testEnvironment = {
    ...
    SEPAY_WEBHOOK_API_KEY:
      process.env.E2E_SEPAY_WEBHOOK_API_KEY ||
