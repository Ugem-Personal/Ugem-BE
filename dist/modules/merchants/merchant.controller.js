...
export const getMyMerchant = asyncHandler(async (req, res) => {
    const merchantId = req.user?.MerchantId;
    if (!merchantId) {
        throw new AppError(403, "Hồ sơ Merchant chưa được phê duyệt");
    }
...
export const updateMyMerchant = asyncHandler(async (req, res) => {
    const merchantId = req.user?.MerchantId;
    if (!merchantId) {
        throw new AppError(403, "Hồ sơ Merchant chưa được phê duyệt");
    }
    const merchant = await merchantService.updateMyMerchant(merchantId, req.body);
    return sendSuccess(res, {
        message: "Cập nhật hồ sơ Merchant thành công",
        data: merchant,
    });
});
export const getMerchantsForMap = asyncHandler(async (req, res) => {
...
export const getMyMerchantViews = asyncHandler(async (req, res) => {
...
export const getMyMerchantStatistics = asyncHandler(async (req, res) => {
    const result = await merchantService.getMyMerchantStatistics(getMerchantId(req));
    return sendSuccess(res, {
        message: "Lấy thống kê Merchant thành công",
        data: result,
    });
});
export const getStaffMerchants = asyncHandler(async (req, res) => {
