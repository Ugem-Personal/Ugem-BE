import { CheckInStatus, NotificationType, OrderStatus, Prisma, } from "../../generated/prisma/client.js";
...
const mapReview = (review) => {
    ...
    const isVerifiedDiner = review.checkIn?.status === CheckInStatus.Verified ||
...
const createCheckInReview = async (customerId, input) => {
    if (!input.checkInId) {
    ...
    if (checkIn.status !== CheckInStatus.Verified ||
        !checkIn.verifiedAt) {
        throw new AppError(403, "Chỉ có thể đánh giá từ lượt ghé đã được xác minh");
    }
    if (input.merchantId &&
        input.merchantId !== checkIn.merchantId) {
        throw new AppError(400, "Merchant ID không khớp với Check-in");
    }
    ...
    if (existingReview) {
        throw new AppError(409, "Lượt ghé này đã được đánh giá");
    }
    let reviewResult;
    try {
        reviewResult = await prisma.$transaction(async (transaction) => {
            ...
            await updateMerchantRating(transaction, checkIn.merchantId);
    ...
    await createNotification({
        ...
        message: `Khách hàng đã đánh giá ${checkIn.merchant.name} ${review.rating}/5 sao.`,
 ...
 */
const createLegacyOrderReview = async (customerId, input) => {
    if (!input.orderId) {
