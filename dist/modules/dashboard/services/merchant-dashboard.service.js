...
export const getMerchantOrderPerformanceByYear = async (merchantId, year) => {
    const merchant = await prisma.merchant.findUnique({
        where: {
            id: merchantId,
        },
...
export const getMerchantDailyRevenue = async (merchantId, startDateValue, endDateValue) => {
    const merchant = await prisma.merchant.findUnique({
        where: {
            id: merchantId,
        },
    ...
    const rejectionRate = summary.totalOrders > 0
        ? Number(((summary.rejectedOrders / summary.totalOrders) * 100).toFixed(2))
        : 0;
    const highestRevenueDay = summary.totalRevenue > 0
