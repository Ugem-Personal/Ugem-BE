...
export const getStaffOrderPerformanceByYear = async (year) => {
    const { startDate, endDate } = getYearRange(year);
    const orders = await prisma.order.findMany({
        where: {
            createdAt: {
    ...
    const averageOrderValue = summary.completedOrders > 0
        ...
        : 0;
    const bestMonth = summary.totalOrders > 0
        ? months.reduce((currentBest, month) => month.revenue > currentBest.revenue ? month : currentBest, months[0])
        : null;
    const worstMonth = summary.totalOrders > 0
        ? months
            .filter((month) => month.totalOrders > 0)
            .reduce((currentWorst, month) => month.rejectionRate > currentWorst.rejectionRate
            ? month
            : currentWorst)
        : null;
    return {
...
export const getStaffDailyRevenue = async (startDateValue, endDateValue) => {
    ...
    const averageOrderValue = summary.completedOrders > 0
        ? Number((summary.totalRevenue / summary.completedOrders).toFixed(2))
        : 0;
    const highestRevenueDay = summary.totalOrders > 0
        ? days.reduce((currentHighest, day) => day.revenue > currentHighest.revenue ? day : currentHighest, days[0])
        : null;
    const highestOrderDay = summary.totalOrders > 0
