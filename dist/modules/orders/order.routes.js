...
orderRouter.post("/sepay/webhook", webhookRateLimiter, authenticateSepayWebhook, validate(sepayWebhookSchema), processSepayWebhook);
orderRouter.use(authenticate);
orderRouter.get("/", requireApprovedMerchant, validate(orderListSchema), getMerchantOrders);
orderRouter.post("/", authorizeRoles("Customer", "Reviewer"), validate(createOrderSchema), createOrder);
orderRouter.get("/mine", authorizeRoles("Customer", "Reviewer"), validate(orderListSchema), getMyOrders);
orderRouter.get("/merchant", requireApprovedMerchant, validate(orderListSchema), getMerchantOrders);
orderRouter.get("/bill", authorizeRoles("Customer", "Reviewer"), validate(getBillSchema), getCustomerBills);
orderRouter.patch("/bill", requireApprovedMerchant, validate(submitBillSchema), submitBill);
