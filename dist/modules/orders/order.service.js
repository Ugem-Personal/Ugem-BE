...
export const mapOrder = (order) => {
    ...
    return {
        ...
        deliveryLatitude: order.deliveryLatitude !== null ? Number(order.deliveryLatitude) : null,
        deliveryLongitude: order.deliveryLongitude !== null ? Number(order.deliveryLongitude) : null,
...
export const createOrder = async (customerId, input, idempotencyKey) => {
    ...
    if (input.paymentMethod === "BankTransfer" ||
        ...
        if (!merchantPaymentConfig?.bankTransferEnabled ||
            !merchantPaymentConfig.bankCode ||
            !merchantPaymentConfig.bankAccountNumber ||
            !merchantPaymentConfig.bankAccountName) {
            throw new AppError(409, "Quán chưa cấu hình tài khoản nhận chuyển khoản");
        }
    }
