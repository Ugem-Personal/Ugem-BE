...
export const getMyOrders = asyncHandler(async (req, res) => {
    const result = await orderService.getMyOrders(getCustomerId(req), getListQuery(req));
    return sendSuccess(res, {
...
export const createMerchantOrder = asyncHandler(async (req, res) => {
    const customerId = req.body.customerId;
    if (typeof customerId !== "string") {
        throw new AppError(400, "Thiếu customerId");
    }
    const order = await orderService.createMerchantOrder(getMerchantId(req), customerId, req.body);
    return sendSuccess(res, {
        statusCode: 201,
...
export const getMerchantOrders = asyncHandler(async (req, res) => {
    const result = await orderService.getMerchantOrders(getMerchantId(req), getListQuery(req));
    return sendSuccess(res, {
...
export const updateOrderStatus = asyncHandler(async (req, res) => {
    const order = await orderService.updateOrderStatus(getMerchantId(req), getParam(req, "id"), req.body);
    return sendSuccess(res, {
...
export const updateCustomerOrderStatus = asyncHandler(async (req, res) => {
    const order = await orderService.updateCustomerOrderStatus(getCustomerId(req), getParam(req, "id"), req.body);
    return sendSuccess(res, {
        message: "Customer cập nhật trạng thái order thành công",
        data: order,
    });
});
export const acceptMerchantOrder = asyncHandler(async (req, res) => {
    const order = await orderService.updateOrderStatus(getMerchantId(req), getParam(req, "orderId"), {
        status: "Accepted",
    });
    return sendSuccess(res, {
...
export const rejectMerchantOrder = asyncHandler(async (req, res) => {
    const order = await orderService.updateOrderStatus(getMerchantId(req), req.body.orderId, {
        status: "Rejected",
...
export const updateOrderStatusByRole = asyncHandler(async (req, res) => {
    const role = req.user?.Role;
    const orderId = getParam(req, "id");
    if (role === "Merchant") {
        const order = await orderService.updateOrderStatus(getMerchantId(req), orderId, req.body);
        return sendSuccess(res, {
    ...
    if (role === "Customer" || role === "Reviewer") {
        const order = await orderService.updateCustomerOrderStatus(getCustomerId(req), orderId, req.body);
        return sendSuccess(res, {
            message: "Customer cập nhật trạng thái order thành công",
            data: order,
        });
