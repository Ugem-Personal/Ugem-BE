...
const calculateFinancialSummary = (orders) => {
    const totalRevenue = orders.reduce((sum, order) => sum + Number(order.finalPrice), 0);
    const reviewerFee = orders.reduce((sum, order) => sum + Number(order.reviewerCommission), 0);
    const platformFee = roundMoney(totalRevenue * (env.PLATFORM_FEE_PERCENT / 100));
    const merchantReceive = roundMoney(Math.max(totalRevenue - platformFee - reviewerFee, 0));
    const averageOrderValue = orders.length > 0 ? roundMoney(totalRevenue / orders.length) : 0;
    return {
...
const calculateRevenueGrowth = (currentRevenue, previousRevenue) => {
    ...
    return roundMoney(((currentRevenue - previousRevenue) / previousRevenue) * 100);
};
const getPeriodStart = (date, periodType) => {
    switch (periodType) {
        case "Day":
...
export const getAdminDashboard = async () => {
    ...
    const [totalUsers, totalMerchants, totalOrders, completedPaidOrders, newUsersToday, pendingApplications, pendingReviewerApplications,] = await prisma.$transaction([
        prisma.user.count(),
    ...
    const totalReviewerFee = completedPaidOrders.reduce((total, order) => total + Number(order.reviewerCommission), 0);
    const totalPlatformFee = Number((totalRevenue * (env.PLATFORM_FEE_PERCENT / 100)).toFixed(2));
...
export const getMerchantRevenues = async (query) => {
    const pageIndex = query.pageIndex;
    const pageSize = query.pageSize;
    const skip = (pageIndex - 1) * pageSize;
    const searchTerm = query.searchTerm?.trim() || undefined;
    const { previousMonthStart, currentMonthStart, nextMonthStart } = getMonthRanges();
    ...
    return merchants.map((merchant) => {
        const completedPaidOrders = merchant.orders.filter((order) => order.status === OrderStatus.Completed &&
        ...
        const previousMonthRevenue = completedPaidOrders
            .filter((order) => {
            const revenueDate = order.completedAt ?? order.orderedAt;
            return (revenueDate >= previousMonthStart && revenueDate < currentMonthStart);
        })
            .reduce((sum, order) => sum + Number(order.finalPrice), 0);
        const lastOrderAt = completedPaidOrders
        ...
        return {
            ...
            ...financial,
            lastOrderAt,
            revenueGrowth: calculateRevenueGrowth(currentMonthRevenue, previousMonthRevenue),
        };
    });
};
export const getMerchantRevenueDetail = async (merchantId, periodType) => {
    ...
    const cancelledOrders = merchant.orders.filter((order) => order.status === OrderStatus.Cancelled ||
        order.status === OrderStatus.Rejected).length;
    const cancellationRate = merchant.orders.length > 0
        ? roundMoney((cancelledOrders / merchant.orders.length) * 100)
        : 0;
    const totalUniqueCustomers = new Set(completedPaidOrders.map((order) => order.customerId)).size;
    const lastOrderAt = completedPaidOrders
        .map((order) => order.completedAt ?? order.orderedAt)
        .sort((first, second) => second.getTime() - first.getTime())[0] ?? null;
