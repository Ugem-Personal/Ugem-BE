import { NotificationType, SupportTicketStatus, UserRole, } from "../../generated/prisma/client.js";
...
function assertMerchantAccess(ticket, merchantId) {
    if (ticket.merchantId !== merchantId) {
        throw new AppError(403, "Bạn không có quyền truy cập yêu cầu này");
    }
}
async function notifyMerchant(merchantUserId, ticketId, title, message) {
...
export const createTicket = async (merchantId, userId, input) => {
...
export const getMerchantTickets = async (merchantId, query) => {
...
export const updateStaffStatus = async (staffUserId, id, input) => {
    const ticket = await getTicketOrThrow(id);
    const updated = await prisma.supportTicket.update({
        where: { id },
        data: {
            ...
            resolvedAt: input.status === SupportTicketStatus.Resolved ||
    ...
    await notifyMerchant(ticket.merchant.userId, id, "Trạng thái yêu cầu hỗ trợ đã cập nhật", `${ticket.subject}: ${input.status}`);
    return mapTicket(updated);
};
export const assignStaff = async (staffUserId, id) => {
    const ticket = await getTicketOrThrow(id);
    const updated = await prisma.supportTicket.update({
        where: { id },
        data: {
