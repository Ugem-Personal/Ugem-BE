...
export const redeemVoucher = async (customerId, voucherTier) => {
    const customer = await prisma.customer.findUnique({
        where: { id: customerId },
    ...
    if (currentPoints < tier.pointsCost) {
        throw new AppError(400, `Bạn cần tối thiểu ${tier.pointsCost} điểm để đổi voucher này (điểm hiện có: ${currentPoints}).`);
    }
...
export const getMyRedeemedVouchers = async (customerId) => {
    ...
    const usedCodes = new Set(usedOrders
