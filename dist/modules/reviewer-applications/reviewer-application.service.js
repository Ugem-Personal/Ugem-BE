...
export const createReviewerApplication = async (customerId, input) => {
...
export const getMyReviewerApplication = async (customerId) => {
...
export const getReviewerApplications = async (query) => {
...
export const reviewReviewerApplication = async (applicationId, reviewerUserId, input) => {
    ...
    if (input.status === "Rejected") {
        const rejected = await prisma.reviewerApplication.update({
            ...
            data: {
                ...
                rejectionReason: input.rejectionReason?.trim() || "Đơn đăng ký không đạt yêu cầu",
        ...
        await createNotification({
            ...
            message: `Đơn đăng ký Reviewer của bạn đã bị từ chối. Lý do: ${rejected.rejectionReason ?? "Không đạt yêu cầu"}`,
...
export const updateReviewerApplication = async (customerId, input) => {
    const application = await prisma.reviewerApplication.findFirst({
        where: {
    ...
    const updated = await prisma.reviewerApplication.update({
        ...
        data: {
            motivation: input.motivation !== undefined ? input.motivation.trim() : undefined,
