...
const run = async () => {
    ...
    if (existingUser) {
        if (existingUser.role !== UserRole.Admin) {
            throw new Error("Email đã thuộc một tài khoản không phải Admin. Script từ chối tự nâng quyền.");
        }
        if (!existingUser.isActive) {
            throw new Error("Admin đã tồn tại nhưng đang bị khóa. Hãy xử lý trạng thái tài khoản riêng.");
        }
