...
const validatePassword = () => {
    if (password.length < 12 ||
        ...
        !/[A-Z]/.test(password) ||
        !/[0-9]/.test(password)) {
        throw new Error("UAT_SEED_PASSWORD phải có ít nhất 12 ký tự, chữ hoa, chữ thường và chữ số");
    }
};
const ensureNonProduction = async () => {
    ...
    const databases = await prisma.$queryRaw `SELECT current_database() AS "databaseName"`;
    const databaseName = databases[0]?.databaseName;
    if (!databaseName)
        throw new Error("Không xác định được database hiện tại");
    return databaseName;
};
const upsertUser = async (input) => prisma.user.upsert({
...
const run = async () => {
    ...
    const category = await ensureCategory("Món Việt UAT", "Danh mục dữ liệu mẫu dành cho UAT.");
