...
export function calculatePreferenceScore(preferences, merchant) {
    if (!preferences) {
    ...
    if (preferences.preferredRestaurantTypes.length > 0) {
        totalWeight += 25;
        if (matchesAnyPreference(preferences.preferredRestaurantTypes, merchant.restaurantType)) {
            matchedWeight += 25;
        }
    }
    if (preferences.preferredCategoryIds.length > 0) {
        ...
        if (preferences.preferredCategoryIds.some((categoryId) => merchantCategoryIds.has(categoryId))) {
            matchedWeight += 50;
        }
    }
    else if (preferences.preferredMainDishTypes.length > 0) {
        totalWeight += 50;
        if (matchesAnyPreference(preferences.preferredMainDishTypes, merchant.mainDishType)) {
            matchedWeight += 50;
        }
    }
    if (preferences.preferredPriceRanges.length > 0) {
        totalWeight += 25;
        if (matchesAnyPreference(preferences.preferredPriceRanges, merchant.priceRange)) {
            matchedWeight += 25;
        }
    }
    return {
