import { MerchantTrafficSource, MonetizationFeeType, } from "../../generated/prisma/client.js";
import { prisma } from "../../config/prisma.js";
const ORGANIC_VIEW_SOURCES = new Set([
    MerchantTrafficSource.Recommendation,
    MerchantTrafficSource.Search,
    MerchantTrafficSource.Map,
    MerchantTrafficSource.Direct,
]);
const toRate = (numerator, denominator) => denominator > 0 ? Number(((numerator / denominator) * 100).toFixed(2)) : 0;
export const getMerchantAnalytics = async (merchantId, range = {}) => {
    ...
    if (!merchant)
        return null;
    const occurredAt = range.from || range.to
        ? { gte: range.from, lte: range.to }
        : undefined;
    const createdAt = occurredAt;
    const [acquisitionEvents, viewRows, saves, reviewCount, feePolicy] = await Promise.all([
    ...
    const visitorsByCustomer = new Map();
    for (const event of acquisitionEvents) {
        visitorsByCustomer.set(event.customerId, (visitorsByCustomer.get(event.customerId) ?? 0) + 1);
    }
    const uniqueVisitors = visitorsByCustomer.size;
    const repeatVisitors = [...visitorsByCustomer.values()].filter((count) => count > 1).length;
    const repeatVisits = Math.max(verifiedVisits - uniqueVisitors, 0);
    const visitsBySource = acquisitionEvents.reduce((result, event) => {
        const source = event.source ?? "Unknown";
        result[source] = (result[source] ?? 0) + 1;
        return result;
    }, {});
    const viewsBySource = Object.fromEntries(viewRows.map((row) => [row.source, row._count._all]));
    const totalViews = viewRows.reduce((total, row) => total + row._count._all, 0);
    ...
    const campaignVerifiedVisits = acquisitionEvents.filter((event) => event.campaignId !== null).length;
    const nonCampaignVerifiedVisits = acquisitionEvents.filter((event) => event.campaignId === null).length;
    ...
    return {
        ...
        conversion: {
            ...
            sponsoredConversionRate: toRate(campaignVerifiedVisits, sponsoredViews),
